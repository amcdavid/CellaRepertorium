...
bulk_y = left_join(bulk_y, filter(glms_y, term == 'popb6'), by = c("cdr3", "v_gene", "j_gene"))
...
bulk_z = left_join(bulk_z, filter(glms_z, term == 'popb6'), by = c("cdr3", "v_gene", "j_gene"))

join = left_join(bulk_y, bulk_z, by = c('pop', 'cdr3', 'v_gene', 'j_gene'), suffix = c('_y', '_z'))

join = join %>% mutate(is_sig = case_when(p.value_y < .05 & p.value_z < .05 ~ 'both',
                                          p.value_y < .05 ~ 'y',
                                          p.value_z < .05 ~ 'z',
                                          TRUE ~ 'neither'
                                          ))

ggplot(data = join, aes(y=n_class_y/total_y, x = n_class_z/total_z, color = is_sig)) + geom_point()
