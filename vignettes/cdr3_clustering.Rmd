...
Average Levenstein distance of CDR3 within each pair

# Some simple phylogenetic relationship
...
Pull the fields listed in `contig_fields` into the `cluster_tbl`, using the values found in the medoid contig
...
```{r, eval = FALSE}
pairing_list = pairing_tables(oligo_clusters_all %>% select(representative, dataset, barcode, chain, umis, reads), cluster_idx = 'representative', cell_identifiers = c('dataset', 'barcode'), canonicalize_fun = canonicalize_by_prevalence, table_order = 2, orphan_level = 1, min_expansion = 4, feature_tbl = feature_tbl, cell_tbl = good_cells, cluster_whitelist = filter(oligo_clusters, n_cluster>8) %>% dplyr::select(cluster_idx.1 = representative) %>% unique())
<<plot_expanded>>
```
...
We end up with a convergence warning.  This is not a suprise, because the `samples` aren't actually replicates -- they are just subsamples drawn for illustrative purposes.
The Balbc mice  have .5 fewer nucleotides per contig, on average, 
