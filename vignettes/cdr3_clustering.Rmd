...
And by other features of the contigs. Here we cluster each contig based on the chain and V-J genes.  This gives us the set of observed V-J pairings:
...
```{r}
aa80 = CellaRepertorium:::canonicalize_cluster(aa80, c('cdr3'), contig_fields = c('cdr3', 'cdr3_nt', 'chain', 'v_gene', 'd_gene', 'j_gene'))
```
...
```{r expanded_clones}
class_colors = data_frame(chain =  unique(aa80$cluster_tbl$chain)) %>% mutate(class_color =  RColorBrewer::brewer.pal(length(chain),"Set1")[seq_along(chain)])

aa80$cluster_pk = 'representative'
pairing_list = pairing_tables(aa80, table_order = 2, orphan_level = 1, min_expansion = 2, cluster_keys = c('cdr3', 'representative', 'chain', 'v_gene', 'j_gene', 'avg_distance'))
...
As was suggested by the histogram, there doesn't seem to be an obvious `pop` effect.
