...
## Note that I'm not confident that the `Clustering` should be an element of the ContigCellDB anymore becase I think that only certain outputs of the clustering may belong in the contig_tbl or cell_tbl
# And because I think that these will be common steps
aa100 = cluster_tbls(cdb)$AA100
aa100 = fine_clustering(aa100)
...
ggplot(aa100$cluster_tbl %>% filter(n_cluster>1) %>% gather(key, value, -cluster_idx) , aes(x = value))+ facet_wrap(~key, scales = 'free') + geom_histogram() + scale_y_sqrt()
...
```{r, results = 'hide'}
cdb = CellaRepertorium:::cdhit_ccdb(cdb, 'cdr3_nt', type = 'DNA', cluster_tbl_name = 'DNA97', identity = .965, min_length = MIN_CDR3_AA*3-1, G = 1)

dna97 = cluster_tbls(cdb)$DNA97
dna97 = fine_clustering(dna97, keep_clustering_details = TRUE)

ggplot(dna97$cluster_tbl %>% filter(n_cluster>1) %>% gather(key, value, -cluster_idx) , aes(x = value))+ facet_wrap(~key, scales = 'free') + geom_histogram() + scale_y_sqrt()

cdb = CellaRepertorium:::cluster_germline(cdb, segment_keys = c('v_gene', 'j_gene', 'chain'))
...
dendro_plot = function(clustering, idx, ccdb, method = 'complete'){
    h = filter(clustering$cluster_tbl, !!sym(clustering$cluster_pk) == idx) %>% pull(fc) %>% .[[1]]
    quer = filter(clustering$contig_tbl, !!sym(clustering$cluster_pk) == idx)
    ann = left_join(quer, ccdb$contig_tbl, by = ccdb$contig_pk)
    hc = hclust(as.dist(h$distance), method = method) %>% dendro_data(type = "rectangle")
    hc$labels = cbind(hc$labels, ann)
   ggplot(hc$segments, aes(x=x, y=y)) + geom_segment(aes(xend=xend, yend=yend)) + 
  theme_classic() + geom_text(data = hc$labels, aes(color = sample, label = fancy_name), size = 3, angle = 90, vjust = 0) + scale_x_continuous(breaks = NULL) + ylab('AA Distance') + xlab('')
}


to_plot = filter(dna97$cluster_tbl, n_cluster > 3)
map(to_plot$cluster_idx, ~ dendro_plot(dna97, ., cdb) + ylab("DNA Distance"))
